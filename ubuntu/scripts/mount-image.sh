...
main() {
  local image_file=$1
  local root_mountpoint=$2
  local boot_mountpoint=$3
  ...
  mount_image_partitions $device $root_mountpoint $boot_mountpoint
  mount_system_partitions_for_chroot $root_mountpoint
  display_mounted_partitions $device $root_mountpoint
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    --image)
      ...
      ;;
    --root-mountpoint)
      ROOT_MOUNTPOINT="$2"; shift
      ;;
    --boot-mountpoint)
      BOOT_MOUNTPOINT="$2"; shift
      ;;
    *)
...
if [[ -z "$IMAGE_FILE" || -z "$ROOT_MOUNTPOINT" ]]; then
  echo "All parameters required (--image, --system-root, --boot-root)"
  exit 1
fi

main $IMAGE_FILE $ROOT_MOUNTPOINT $BOOT_MOUNTPOINT 2>&1
